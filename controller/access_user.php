...
ini_set('display_errors', '1');
	...
	if($exist){
		...
		if($habilitado == 1 || $ok){ // si el sitio no esta bloqueado o el usuario tiene permiso de configuracion maestra
			//$result = Repository_User::get_user($user);//obtengo la informacion del usuario

			if ($user_data[0][4] == 0 ){//$result[0][4] == 0) { // si el usuario no esta activos redirecciona al login.
