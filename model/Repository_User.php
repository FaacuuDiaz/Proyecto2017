...
class Repository_User
{
    ...
    public static function check_existe($user, $email)
    {
        $con     = Connection::open_connection();
        $consult = "SELECT * FROM usuario WHERE username=:user or email=:email";
        $sen     = $con->prepare($consult);
        $sen->bindParam(':user', $user);
    ...
    public static function check_update($user, $email, $id)
    {
        $con     = Connection::open_connection();
        $consult = "SELECT * FROM usuario WHERE (username=:user or email=:email) and id!=:id";
        $sen     = $con->prepare($consult);
        $sen->bindParam(':user', $user);
        $sen->bindParam(':email', $email);
        $sen->bindParam(':id', $id);
